...
export const meetingRecordService = {
     ...
     * 
     * IMPORTANT: meetingId is ALWAYS generated by backend as `{roomName}_{uuid}`
     * Frontend sends roomName, backend generates unique meetingId
     */
    async processMeetingStarted(payload: MeetingStartedPayload): Promise<void> {
        const validation = validateRequiredFields(payload, [
            'meetingId', // This is actually roomName from frontend
            'roomName',
            'startedAt',
        ])
        ...
        // No existing active/scheduled meeting - always create NEW meeting with UUID
        // meetingId format: {roomName}_{uuid} - ensures uniqueness for multiple sessions
        const newMeetingId = `${payload.roomName}_${crypto.randomUUID()}`
        console.log(`[meetingRecordService] Creating new meeting: ${newMeetingId} (room: ${payload.roomName})`)
        ...
        try {
            ...
            if (prismaError?.code === 'P2002') {
                // Unique constraint violation - extremely rare with UUID, but handle gracefully
                console.log(`[meetingRecordService] UUID collision (rare), regenerating: ${newMeetingId}`)
                const retryMeetingId = `${payload.roomName}_${crypto.randomUUID()}`
                await prisma.meeting.create({
                    data: {
                        id: retryMeetingId,
                        roomName: payload.roomName,
