...
export const meetingRecordService = {
     ...
     * 
     * IMPORTANT: meetingId is ALWAYS generated by backend as `{uuid}`
    ...
    async processMeetingStarted(payload: MeetingStartedPayload): Promise<void> {
        ...
        // meetingId format: {uuid} - ensures uniqueness for multiple sessions
        ...
        try {
            ...
            if (prismaError?.code === 'P2002') {
                ...
                const retryMeetingId = `${crypto.randomUUID()}`
                await prisma.meeting.create({
                    data: {
                        id: retryMeetingId,
                        roomName: payload.roomName,
