...
## Maintenance / recent changes

- Many `Class_*.txt` files under `1-JitsiConference/` have been standardized to a shared English template. Descriptive text was translated from Vietnamese to English where present.
- Important: preserve the CSV blocks (enum/method signature rows) as the canonical source for tooling.
- If you update event names or enum tokens, update the corresponding `JitsiConferenceEvents_Enum.txt` or `*_Errors_Enum.txt` files and run verification before merging.

```JitsiAPI reference: use each folder's`README.md`and the`Class\__`/`_\_Events_Enum.txt` files as the authoritative source for event names, method signatures, and short usage examples. Keep this folder for compact, machine-friendly API reference only.
